...
def _parse_twitter_ts(dt_str):
    """Parse Twitter's created_at format → UTC Unix timestamp (float). Returns None on failure."""
    try:
...
def process_thread(thread_dir, label):
    ...
    Extracts raw features (text, followers, verification, timestamp) and connections.
    """
    G = nx.DiGraph()
    ...
    G.add_node(
        ...
        created_at_ts=_parse_twitter_ts(source_tweet.get("created_at")),
        is_source=True
    )
    ...
    if reactions_dir.exists():
        for reaction_file in reactions_dir.glob("*.json"):
            ...
            G.add_node(
                ...
                created_at_ts=_parse_twitter_ts(reaction_tweet.get("created_at")),
                is_source=False
            )
