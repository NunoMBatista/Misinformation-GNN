...
vs.\ 1.528; $\rho = -0.102$, $p < 0.001$), with small effect sizes.
...
We evaluate four classifiers under three feature configurations on the held-out
test set (3{,}106 graphs). The random-classifier lower bound is macro F1 = 0.375;
Table~\ref{tab:results} reports full results.
...
All three structural-only classifiers fall below the random lower bound
(F1 = 0.200--0.303 vs.\ 0.375): topology alone does not generalise across events.
...
0.610. Adding structural features consistently hurts generalisation: GAT drops from
0.610 to 0.565 and GCN from 0.551 to 0.548 in the full configuration.
...
All pairwise McNemar comparisons ($p < 0.001$): message passing beats MLP,
GAT beats GCN, and adding structural features hurts both GCN and GAT.
The GNN-MLP F1 margin is modest (+0.031 text, +0.027 full), suggesting NLP
embeddings carry the bulk of the discriminative signal.
